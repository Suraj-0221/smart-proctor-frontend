...
export default function Dashboard() {
  ...
  React.useEffect(() => {
    const historical = JSON.parse(localStorage.getItem('sp_historical_sessions') || '[]');
    if (historical.length > 0) {
      // Formatted list for dashboard
      const formatted = historical.map((h) => ({
        id: h.id || 'S00X',
        name: h.name || 'Unknown',
        exam: h.exam || 'Unknown',
        risk: h.riskScore !== undefined ? h.riskScore : 0,
        violations: h.violationsCount || 0,
        date: h.dateStr || 'Unknown',
        status: h.status || 'passed'
      }));
      setSessions([...formatted, ...MOCK_SESSIONS]); // show new ones at top
    }
  }, []);

  const totalSessions = sessions.length;
  const flagged       = sessions.filter(s => s.status === 'flagged').length;
  const avgRisk       = totalSessions > 0 ? Math.round(sessions.reduce((a, s) => a + s.risk, 0) / totalSessions * 100) : 0;
