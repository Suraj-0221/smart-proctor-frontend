...
export default function ExamPage() {
  ...
  const handleSubmit = () => {
    ...
    const historyData = JSON.parse(localStorage.getItem('sp_historical_sessions') || '[]');
    const newHistorySession = {
      ...sessionData,
      id: `S${String(historyData.length + 1).padStart(3, '0')}`,
      status: riskScore >= 0.6 ? 'flagged' : riskScore >= 0.3 ? 'review' : 'passed',
      risk: riskScore,
      violationsCount: violations.length,
      dateStr: new Date().toLocaleDateString()
    };
    historyData.unshift(newHistorySession);
    localStorage.setItem('sp_historical_sessions', JSON.stringify(historyData));
  };
